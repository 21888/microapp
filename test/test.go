...
func Setup() {
	onceSetup.Do(func() {
		...
		MockMicroApp = microapp.New(microapp.Config{
			AppId:     "APPID",
			AppSecret: "SECRET",
		})
		...
		microapp.ServerUrl = MockSvr.URL // 拦截发往服务器的请求

		// Mock access token
		MockSvrHandler.HandleFunc("/api/apps/token", func(w http.ResponseWriter, r *http.Request) {
